...
class Enemy4 {
  ...
  update() {
    this.x = canvas.width/3 * Math.sin(this.angle * Math.PI / this.curve) + (canvas.width / 2 - this.width / 2);
    this.y = canvas.height/3 * Math.cos(this.angle * Math.PI / 350) + (canvas.height / 2 - this.height / 2);
    this.angle += this.angleSpeed;
    if (this.x + this.width < 0) this.x = canvas.width;
    // animate sprite
    if (gameFrame % this.flapSpeed === 0) {
